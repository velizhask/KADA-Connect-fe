...
const KadaTermsOfService = () => {
  ...
  const parseMarkdown = (markdown: string): string => {
    ...
    // Ordered lists
    html = html.replace(/^\d+\.\s+(.+)$/gim, '<li class="text-gray-700 mb-2 ml-6">$1</li>');
    html = html.replace(/(<li class="text-gray-700 mb-2 ml-6">.*<\/li>\n?)+/g, '<ol class="list-decimal list-inside space-y-2 mb-4">$&</ol>');
    
    // Unordered lists
    html = html.replace(/^[-*]\s+(.+)$/gim, '<li class="text-gray-700 mb-2 ml-6">$1</li>');
    html = html.replace(/(<li class="text-gray-700 mb-2 ml-6">.*<\/li>\n?)+/g, '<ul class="list-disc list-inside space-y-2 mb-4">$&</ul>');
